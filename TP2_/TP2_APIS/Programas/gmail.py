...
def obtener_datos_mails(id_mails:list, servicio:Resource) -> dict:
    ...
    for id_mail in id_mails:
        try:
            ...
            lectura_mail = servicio.users().messages().get(userId='direccion_de_mail@gmail.com', id = id_mail).execute()
...
def marcar_como_leido(servicio:Resource, datos_emails:dict) -> None:
    ...
    for id_mail in datos_emails:
        conexion = servicio.users().messages().modify(userId='direccion_de_mail@gmail.com', id=id_mail,
...
def obtener_ids_mails(servicio:Resource, fecha_actual:int) -> list:
    ...
    try:
        emails_recibidos = servicio.users().messages().list(userId='direccion_de_mail@gmail.com', q=f'label: inbox has:attachment is:unread').execute()
...
def enviar_mails(servicio:Resource, entregas:list, asunto:str, cuerpo:str) -> None:
    ...
    if len(entregas) == 0:
        ...
        for mail in entregas:
            ...
            try:
                mensaje = servicio.users().messages().send(userId = "direccion_de_mail@gmail.com", 
...
def obtener_archivos_adjuntos(servicio:Resource, datos_entrega_correcta:dict, datos_emails:dict) -> list:
    ...
    for indice in range(len(id_mails)):
        archivo_adjunto = servicio.users().messages().attachments().get(userId='direccion_de_mail@gmail.com',
